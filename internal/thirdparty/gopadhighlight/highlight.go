...
// injectionForMatch resolves the injected language name and content node for a
// query match.
//
// PATCHED (jackieli.dev fork): upstream bailed out here whenever
// config.InjectionLanguageCaptureIndex was nil, which makes injection.language
// resolution work only for queries that capture the language as a node (e.g.
// `@injection.language`). But the far more common tree-sitter idiom — used by
// gsx's own injections.scm, and by most real-world grammars — sets the
// language purely via a `(#set! injection.language "go")` property with no
// accompanying capture. Upstream's early return meant that style of query
// silently produced zero injections. We only require a content capture here;
// the language can still come from the property-settings loop below, matching
// the reference tree-sitter-highlight (Rust) behavior this package ports.
// See: https://github.com/gopad-dev/go-tree-sitter-highlight (no fix upstream
// as of the pinned commit 3ffb64c3a6501b10655eb9eafa0a19e37104ab41).
func injectionForMatch(config Configuration, parentName string, query *tree_sitter.Query, match tree_sitter.QueryMatch, source []byte) (string, *tree_sitter.Node, bool) {
	if config.InjectionContentCaptureIndex == nil {
		return "", nil, false
	}
