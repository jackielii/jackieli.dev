...
func newIterLayers(
	...
	for {
		if err := highlighter.Parser.SetIncludedRanges(ranges); err == nil {
			...
			if config.CombinedInjectionsQuery != nil {
				...
				for {
					...
					// PATCHED (jackieli.dev fork): upstream had this
					// condition inverted (`== ""`), so a resolved language
					// name (e.g. "javascript" for <script>) was never
					// actually recorded — it only "recorded" the empty
					// string, which is what the zero value already was.
					// That silently made every combined injection
					// (<script>, <style>, js`...`, css`...`) a no-op.
					if languageName != "" {
			...
			// PATCHED (jackieli.dev fork): upstream did `continue` here, which
			// jumps back to the top of this `for {}` without ever popping the
			// next item off `queue` (that happens at the bottom of the loop
			// body, below). Since `config`/`ranges` are unchanged, a config
			// whose query has zero captures over its range — e.g. an injected
			// Go layer over an empty `parameter_list` like `()` — reparses the
			// exact same input forever. We just skip appending the (useless,
			// capture-less) layer instead, and let control fall through to the
			// existing queue-draining logic below so the next queued injection
		...
		// PATCHED (jackieli.dev fork): upstream wrote
		// `append(queue, queue[1:]...)`, which appends queue's own tail back
		// onto itself instead of dropping the just-consumed front item — the
		// queue never shrinks (it roughly doubles each iteration), so any
		// combined injection (<script>, <style>, js`...`, css`...`) that
		// enqueues work here reprocesses the same item forever and grows
		// without bound. The fix is the obvious "pop the front" op.
